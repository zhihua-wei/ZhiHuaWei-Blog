...
class Login extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($flag) {
            ...
            $success['url'] = site_url("Weiadmin/Admin/index");
            $success['wait'] = 3;
            $data['success'] = $success;
            $this->load->view('Weiadmin/success.html', $this->weiDate);
        } else {
            $this->load->view('Weiadmin/login.html', $this->weiDate);
        }
    }
    ...
    public function singin()
    {
        ...
        if ($verify_code === $code) {
            ...
            if ($admin) {
                ...
                $this->wei->add_log('登录成功！', $admin['admin_id'], $admin['username']);
                redirect('Weiadmin/Admin/index');
            } else {
                $error['msg'] = "用户名密码输入错误，请重新输入！";
                $error['url'] = site_url("Weiadmin/Login/index");
                $error['wait'] = 3;
                $data['error'] = $error;
                $this->load->view('Weiadmin/error.html', $data);
            }
        } else {
