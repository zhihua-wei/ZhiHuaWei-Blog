...
class Personal extends Wei_Controller
{
    ...
    public function updateprofile()
    {
        ...
        if (!empty($_FILES['head_pic']['tmp_name'])) {
            ...
            if ($this->upload->do_upload('head_pic')) {
                ...
                $error['url'] = site_url("Weiadmin/Personal/index");
                $error['wait'] = 3;
                $data['error'] = $error;
    ...
    public function changepwd()
    {
        ...
        if ($_POST) {
            ...
            if ($newpwd === $confirm_password) {
                if ($this->wei->seach_admin_by($this->ADMINISTRSTORS['admin_id'], $oldpwd)) {
                    if ($this->wei->set_admin_password($this->ADMINISTRSTORS['admin_id'], $newpwd)) {
                        delete_cookie('auth');
                        ...
                        $error['msg'] = "系统繁忙，请稍后再试！";
                        $error['url'] = site_url("Weiadmin/Personal/changepwd");
                        $error['wait'] = 3;
                        $data['error'] = $error;
                        $this->load->view('error.html', $data);
                    }
