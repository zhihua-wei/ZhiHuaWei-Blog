...
class Authgroup extends Wei_Controller
{
    ...
    public function del($id)
    {
        ...
        if ($this->ag->get_admincount_of_authgroup($id)) {
            ...
            $error['url'] = site_url("Weiadmin/Authgroup/index");
            $error['wait'] = 3;
            $data['error'] = $error;
        ...
        if ($this->ag->del_auth_group($id)) {
            $this->wei->add_log('删除角色权限组，ID：' . $id, $this->ADMINISTRSTORS['admin_id'], $this->ADMINISTRSTORS['username']);
            $success['msg'] = "角色权限组删除成功！";
            $success['url'] = site_url("Weiadmin/Authgroup/index");
            $success['wait'] = 3;
    ...
    public function update()
    {
        ...
        if ($id) {
            ...
            if ($this->ag->update_auth_group($id, $params)) {
                $this->wei->add_log('修改角色权限组：' . $params['title'], $this->ADMINISTRSTORS['admin_id'], $this->ADMINISTRSTORS['username']);
            ...
            if ($this->ag->insert_auth_group($params)) {
                $this->wei->add_log('新增角色权限组：' . $params['title'], $this->ADMINISTRSTORS['admin_id'], $this->ADMINISTRSTORS['username']);
