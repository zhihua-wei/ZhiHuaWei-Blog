...
class Administrator extends Wei_Controller
{
    ...
    public function index($offset = '')
    {
        ...
        //配置分页信息
        $config['base_url'] = site_url('Weiadmin/Administrator/index/');
    ...
    public function del($id)
    {
        ...
        if ($id == 1) {
            ...
            $error['url'] = site_url("Weiadmin/Administrator/index");
            $error['wait'] = 3;
            $data['error'] = $error;
        ...
        if ($this->ag->del_administrator($id)) {
            $this->wei->add_log('删除管理员用户，ID：' . $id, $this->ADMINISTRSTORS['admin_id'], $this->ADMINISTRSTORS['username']);
            $success['msg'] = "管理员用户删除成功！";
            $success['url'] = site_url("Weiadmin/Administrator/index");
            $success['wait'] = 3;
    ...
    public function addeditadmininfo()
    {
        ...
        if (!empty($_FILES['head_pic']['tmp_name'])) {
            ...
            if ($this->upload->do_upload('head_pic')) {
                ...
                $error['url'] = site_url("Weiadmin/Administrator/add");
                $error['wait'] = 3;
                $data['error'] = $error;
                $this->load->view('error.html', $data);
                return;
        ...
        if ($admin_id) {
            if ($this->ag->update_administrator($admin_id, $auth_group, $params)) {
                $this->wei->add_log('修改管理员信息，管理员ID：' . $admin_id, $this->ADMINISTRSTORS['admin_id'], $this->ADMINISTRSTORS['username']);
            ...
            if ($this->ag->insert_administrator($params, $auth_group)) {
                $this->wei->add_log('添加管理员信息，管理员用户名：' . $params['username'], $this->ADMINISTRSTORS['admin_id'], $this->ADMINISTRSTORS['username']);
